/*
    ...
    Date:      2 June 2021  1:25 pm
    Project 2: Blackjack Game
    Version:   v.2 (Starting array for 4 players hand values, and initial choice
                    to stand or hit after first two cards also ridding of 
                    unnecessary functions for these arrays to work)
 */
...
int calcD3C(int&, int);
int calcD4C(int&, int);
void ptReslt(int [], int, int [], int, int, char []);
    ...
    if((answer == 'y') || (answer == 'Y'))
    {
        ...
        for(int x = 0; x < SIZE; x++)
        {
            if(choices[x] == 's'||choices[x] == 'S')
            {
                ...
                ptReslt(sum1, sum2, bets, win, loss, choices);
                cout << endl;
            }
...
void ptReslt(int sum1 [], int sum2, int bets [], int win, int loss, char choices []){
    for (int k = 0; k < SIZE; k++)
    {
        cout << "Player " << k + 1 <<": \n";
        //Dealer wins by greater
        if(sum2 > sum1[k] && sum2 < 21)
        {
            cout << "You lose...\n";
        ...
        {
            cout << "Your hand holds the total value: " << sum1[k] << ".\n";
        ...
        else if(sum1[k] == 21 && sum1[k] > sum2) 
        {
            cout << "The total value of your hand is " << sum1[k] << ".\n"
                 << "You got BLACKJACK!\n"
                 << "Dealer has a hand with total value: " << sum2 << ".\n"
