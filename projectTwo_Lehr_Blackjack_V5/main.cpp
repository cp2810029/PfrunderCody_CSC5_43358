/*
    ...
    Date:      7 June 2021  10:35 am
    Project 2: Blackjack Game
    Version:   v.5 (Final Version)
 */
...
int calcD3C(int&, int);
int calcD4C(int&, int);
void ptReslt(int, int, vector<int> &, int, float);
...
int main(int argc, char** argv){
    ...
    int    nShuff,   //Number of card shuffles
           ...
           toChs;    //Player chosen with BJ at end of the game for a drink
    ...
    vector<int> bets(SIZE);           //4 players bets array  
    int    plyNum[ ] = {1 , 2, 3, 4}; //Parallel array to bets[] - highest bet
    ...
    toChs = 21;            //Player with blackjack gets a sticker
    ...
    cout << "Would you four ladies/gentlemen like to play a game? "
    ...
    if((answer == 'y') || (answer == 'Y')){
        ...
        cout << fixed << setprecision(2) << showpoint;
        cout << "Highest bet is $"<< static_cast<float>(max) 
             << " and was given by Player " << plyNum[index] << ".\n";
        ...
        for(int x = 0; x < SIZE; x++){
            if(choices[x] == 's'||choices[x] == 'S'){
                ...
                ptReslt(sum1[x], sum2, bets, x, BLACKJACK);
                cout << "-----------------------" << endl
                     << endl;
            }
                ...
                //Not a blackjack 
                    ...
                    //Player chooses not to hit for a fourth card
                        ...
                        //Add dealers third card to hand
                        sum2 = cdOne + cdTwo + cdTres;
                        ptReslt(sum1 [x], sum2, bets, x, BLACKJACK);
                        cout << "-----------------------" << endl;
                    }
        ...
        int rnd = rand()%2+1; //Random number 1 or 2
        if(sqrt(rnd) == 1){
            cout << "The players hands from least to greatest go: \n";
            //Bubble sort players hands least to greatest
            bublSrt(sum1, SIZE);
            // Function to print an array 
            prntArr(sum1, SIZE);
        }
        else{
            cout << "The players hands sorted go: \n";
            //Selection sort players hands least to greatest
            selcSrt(sum1, SIZE);
            // Function to print an array 
            prntArr(sum1, SIZE);
        }
        //Linear search to get a free drink
        for(int x = 0; x < SIZE; x++){
            if(linSrch(sum1, toChs, SIZE)){
                cout << "Player " << x + 1 << " you get a free drink for a "
                     << "well played game." << endl;
            }
        }   
    }
    //Exit stage right
...
void ptReslt(int sum1, int sum2, vector<int> &bets, int k, float BLACKJACK){
    cout << "Player " << k + 1 <<": \n";
    ...
    //Dealer bust
        ...
        bets[k] *= 2.0;
        cout << "Your winnings are: $" << bets[k] << endl;
    }
    ...
    //Player wins
        ...
        cout << "You win!\n"
             ...
             << "Your winnings are: $" << (bets[k]*2) << endl;
    }
...
//Bubble sort players hands least to greatest
void bublSrt(int sum1[], int SIZE){
    bool swap;
    do{
        for(int x = 0; x < SIZE - 1; x++){
            swap = false;
            if(sum1[x] > sum1[x+1]){       //Put the smallest at top of List
                sum1[x] = sum1[x]^sum1[x+1];  //In place Swap
                sum1[x+1] = sum1[x]^sum1[x+1];  //In place Swap
                sum1[x] = sum1[x]^sum1[x+1];  //In place Swap
            swap = true;
            }
        }
...
bool linSrch(int sum1[], int toChs, int SIZE){
    for(int i = 0; i < SIZE; i++){
        if(toChs == sum1[i])return true;
    }
    return false;
}

// Function to print an array 
void prntArr(int sum1[], int SIZE){
    for (int i = 0; i < SIZE; i++){
        cout << sum1[i] << " ";
        cout << endl;
    }
